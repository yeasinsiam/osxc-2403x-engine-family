...
if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	*/
	...
	function Message($id){
		$ci=get_instance();
		$ci->db->select('msg_message');
		$ci->db->where('msg_id', $id);
		$query = $ci->db->get('tbl_messages');

		if($query->num_rows() ==''){
			return false;
		}else{
			return $query->row()->msg_message;
		}
	}

	function loadMessage($activeUser){
		$ci=get_instance();
		$ci->db->select('mbr.cust_id,mbr.cust_fname,mbr.cust_lname,mbr.cust_profile,msg.*');
		$ci->db->join('tbl_members mbr','msg.msg_sender=mbr.cust_id','LEFT');
		$ci->db->where('msg.msg_receiver', $activeUser);
		// $ci->db->or_where('msg.msg_sender', $activeUser);
		...
		if($query->num_rows() ==''){
			...
			if(!empty($recordMessage)){
				...
				if($msg->msg_read=='no'){
					...
					if($msg->msg_receiver==$activeUser ){
				    ...
				    }else if($msg->msg_sender==$activeUser){
				     $senderName = member_name($msg->msg_receiver);
				  $member_profile = member_profile($msg->msg_receiver);
									...
									<div class="noto-content clearfix" style="background:'.$bg.'">
										...
										<div class="noto-text">
											...
											<div class="noto-message">
												<a href='.site_url("account/message/").' style="color:#7a8192">
													'.$notiFyMessage.'
												</a>
											</div>
										</div>
	...
	function message_last($sender){
		$ci=get_instance();
		$ci->db->select('msg_message,msg_created');
		$ci->db->order_by('msg_id','DESC');
		$ci->db->where('msg_sender', $sender);
		$query = $ci->db->get('tbl_messages');
		//echo $ci->db->last_query($query);
		if($query->num_rows() ==''){
			return false;
