...
class Part extends MY_Controller {
	...
	public function __construct() {
		...
		/* === FOR BLOG ===*/ 
		$this->load->model("Part_model","Product"); 
		...
		$this->table="tbl_part";
		/*==== FOR Category ====*/ 		
		$this->table_category="tbl_part_category";
		
    }
  
    public function index() {
		$content['part']=$this->Product->product_list($this->table); 
		$content['subview']="part/product_list";
	...
	public function add(){
		...
		if($REQUESTMETHOD== "POST"){ 
			...
			if($result){
				$this->session->set_flashdata('msg',array('message' => 'Part has been successfully added.','class' => 'success','type'=>'Ok!'));
				redirect('part/add');
			}else{
       ...
       $content['media']=$this->Product->media_list(); 
		...
		$content['subview']="part/add_product"; 
		$this->load->view('layout', $content);	 
	}
	...
	public function edit($id=null){
		...
		if(!empty($content['product_info'])){
			...
			$desc=html_escape($this->input->post('description',FALSE));	
				...
				if($result){
					$this->session->set_flashdata('msg',array('message' => 'Part has been successfully Update.','class' => 'success','type'=>'Ok!'));
					redirect('part/edit/'.$id);
				}else{
					$this->session->set_flashdata('msg',array('message' => 'Unable to Added.Some error occurred.','class' => 'danger','type'=>'Oops!'));
					redirect('part/edit/'.$id);
				}
			...
			$content['subview']="part/edit_product"; 
			$this->load->view('layout', $content);
		}else{
			$content['subview']="part/badrequest";
			$this->load->view('layout', $content);
		} 
	...
	public function delete($id=null){ 
        if($id!==NULL) {
			...
			if($result){
				$this->session->set_flashdata('msg',array('message' => 'Part has been successfully Delete','class' => 'success','type'=>'Ok!'));
				redirect('part');
			}else{
